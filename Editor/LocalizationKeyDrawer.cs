...
namespace ZG_Localization {
    ...
    public class LocalizationKeyDrawer : PropertyDrawer {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            string[] keys = _keys;
            GUIContent[] keyContents = keys.Select(k => new GUIContent(k, GetEnTooltip(k, attr.fileName))).ToArray();

            int selectedIndex = EditorGUI.Popup(position, new GUIContent(label.text, GetEnTooltip(property.stringValue, attr.fileName)),
                currentIndex, keyContents);
